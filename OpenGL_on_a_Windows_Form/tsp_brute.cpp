...
/******************************************************************************/
void tsp_brute::start(Mat input_mat, Mat& sort_index_mat)
{
  double *distance;
  int i1,i2;
  int m, n, more;
  ...
  n = m = input_mat.rows; //dummy node
  distance = ( double * ) malloc ( m * n * sizeof ( double ) );
  ...
  for(int i=0;i<n;i++)
  {
	for(int j=0;j<n;j++)
	{
		...
		distance[i*n+j] = 0.0;
		for(int t=0;t<input_mat.cols;t++)
		{
			distance[i*n+j] += abs( input_mat.at<float>(i,t) - input_mat.at<float>(j,t) );
		}
	}
  }
  ...
  //sort_index_mat = Mat::zeros(n,1,CV_32S);
  printf("Minimum Tour: \n");
  for(int i=0;i<n;i++)
  {
	 printf("%d",p_min[i]);	
	 sort_index_mat.at<int>(i,0) = p_min[i];
	 if(i!=n-1) printf("-");
  }
  printf ("\n");
...
void tsp_brute::tsp_path(Mat input_mat, Mat& sort_index_mat)
{
	int max_dist_node1,max_dist_node2;
	float max_dist = -100;
	for(int i=0;i<input_mat.rows;i++)
	{
		for(int j=0;j<input_mat.rows;j++)
		{
			...
			for(int t=0;t<input_mat.cols;t++)
			{
				dist += abs( input_mat.at<float>(i,t) - input_mat.at<float>(j,t) );
			}
	...
	cout << "max_dist_node1 "<< max_dist_node1 << " max_dist_node2 " << max_dist_node2 << endl;

	start(input_mat,max_dist_node1,max_dist_node2,sort_index_mat);
}


void tsp_brute::start(Mat input_mat, int start, int end, Mat& sort_index_mat)
{
  printf ( "Brute Force TSP from %d to %d\n", start, end);
  double *distance;
  int i1,i2;
  int m, n, more;
  int *p;
  ...
  n = m = input_mat.rows + 1; //dummy node
  distance = ( double * ) malloc ( m * n * sizeof ( double ) );
  ...
  for(int i=0;i<n;i++)
  {
	for(int j=0;j<n;j++)
	{
		...
		{
			if( (i==n-1&&j==start) || (i==start&&j==n-1) || (i==n-1&&j==end) || (i==end&&j==n-1) )
			{
				distance[i*n+j] = 0.1;
			}
			else
			{
				distance[i*n+j] = 10000.0;
			}
		}
		else
		{
  ...
  //Make a tour from start to end and ignore the dummy node
  //EX:tour是0-5-4-2-3-1(0是start,4是end,5是dummy)
  //先把tour index左右都接起來-> (0-5-4-2-3-1)-(0-5-4-2-3-1)-(0-5-4-2-3-1)
  //再判斷start在第二個group的dummy node(5)的前一個還是後一個,若是前一個則往前走,直到碰到下一個dummy node,另一方向同理
  vector<int> tour_concat;
  vector<int> tour_index;
  int dummy_index = n-1;
  static int target_dummy;
  for(int t=0;t<3;t++)
  {
	  for(int i=0;i<n;i++)
	  {
		  tour_concat.push_back(p_min[i]);
		  if(p_min[i]==dummy_index)
		  {
			 target_dummy = i;
		  }
	  }
  ...
  target_dummy += n;
  int current;
  if(tour_concat[target_dummy-1]==start)
  {
	 current = target_dummy-1;
     while(tour_concat[current]!=dummy_index)
	 {
		 tour_index.push_back(tour_concat[current] );
		 current--;
	 }
  }
  else if(tour_concat[target_dummy+1]==start)
  {
	 current = target_dummy+1;
     while(tour_concat[current]!=dummy_index)
	 {
  ...
  printf ("Tour output from start to end: \n");
  //sort_index_mat = Mat::zeros(tour_index.size(),1,CV_32S);
  for(int i=0;i<tour_index.size();i++)
  {
	  printf ("%d ",tour_index[i]);
	  sort_index_mat.at<int>(i,0) = tour_index[i];
  }
  tour_index.clear();
...
int tsp_brute::ch_eqi ( char ch1, char ch2 )
...
int tsp_brute::ch_to_digit ( char ch )
...
int tsp_brute::file_column_count ( char *input_filename )
...
int tsp_brute::file_row_count ( char *input_filename )
...
void tsp_brute::perm_next3 ( int n, int p[], int *more )
...
double *tsp_brute::r8mat_data_read ( char *input_filename, int m, int n )
...
void tsp_brute::r8mat_header_read ( char *input_filename, int *m, int *n )
...
void tsp_brute::r8mat_print ( int m, int n, double a[], char *title )
...
void tsp_brute::r8mat_print_some ( int m, int n, double a[], int ilo, int jlo, int ihi,
  int jhi, char *title )
...
int tsp_brute::s_len_trim ( char *s )
...
double tsp_brute::s_to_r8 ( char *s, int *lchar, int *error )
...
int tsp_brute::s_to_r8vec ( char *s, int n, double rvec[] )
...
int tsp_brute::s_word_count ( char *s )
